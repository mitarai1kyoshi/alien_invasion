...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets):
	...
	if len(aliens) == 0:
		bullets.empty()
		ai_settings.increase_speed() #speed up
		stats.level += 1
		sb.prep_level()
		creat_fleet(ai_settings, screen, ship, aliens)

def get_num_aliens_x(ai_settings, alien_width):
	avaliablespace_x = ai_settings.screen_width - 2 * alien_width
	num_alien_x = int(avaliablespace_x / (2 * alien_width))
	return num_alien_x
	
def get_number_rows(ai_settings, ship_height, alien_height):
	avaliablespace_y = (ai_settings.screen_height - 
		(4 * alien_height) - ship_height)
	number_rows = int(avaliablespace_y / (2 * alien_height))
	return number_rows
